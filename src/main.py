...
from model_training import train_flood_severity_model, train_economic_impact_model, train_water_level_model, evaluate_classification_model, evaluate_regression_model
...
def main():
    ...
    if merged_data is not None:  # Ensure that merging was successful
        preprocessed_data = preprocess_data(merged_data, output_file='data/processed/merged_data.csv')
    
        # Apply feature engineering and save the engineered data
        engineered_data = feature_engineering(preprocessed_data, output_file='data/processed/engineered_data.csv')
